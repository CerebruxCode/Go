...
/*
   ...
   ακολουθούμενος από το τύπο της αποθηκευμένης τιμής. Ο * χρησιμοποιείτε
   ...
   var p * int
   Ο τύπος * T είναι δείκτης σε τιμή Τ. Η μηδενική τιμή του είναι μηδέν.
   ...
   Αυτό είναι που μας επιτρέπει να μετατρέψουμε την αρχική μεταβλητή.
					...
					& <-- Δείξε Προς την διέυθηνση μνήμης μιας μεταβλητής
...
Σε αντίθεση με το C, το Go δεν έχει αριθμητικό pointer.
...
func main() {
	...
	// Πάρτε τη διεύθυνση του x σην οποία δείχνει, με το &
	fmt.Println("Memory Address for x =", &x)

	// Μπορούμε επίσης να δημιουργήσουμε έναν δείκτη με την new
