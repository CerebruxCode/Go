// Κάθε πρόγραμμα Go ξεκινά με μια δήλωση πακέτου που παρέχει έναν τρόπο για
// επαναχρησιμοποίηση του κώδικα. Η 'main' είναι ο "πυρήνας" του προγράμματος απο όπου
// ξεκινάει η εφαρμογή μας. Έτσι αφού εφαρμογές αποτελούνται απο αρχεία κώδικα
...
// Το import επιτρέπει τη χρήση του κώδικα από άλλα πακέτα
// Το πακέτο fmt παρέχει μορφοποίηση για δεδομένα εισόδου και εξόδου
import "fmt"

// <- Βάζοντας αυτό μπορούμε να σχολιάσουμε τον κώδικά μας σε μια γραμμή

/*
 Αν θέλουμε πολλαπλές γραμμές
 σχολίων τότε βάζουμε
 ανάμεσα σε αυτά.
*/

func add(x int, y int) int { // δημιουργούμε μια δικιά μας συνάρτηση με όνομα add

	/*η συνάρτηση add παίρνει 2 ορίματα x και y τύπου intiger και δίνουν έξοδο πάλι int
...
// Οι λειτουργίες - συναρτήσεις του προγράμματός μας ξεκινούν με το όρισμα func και περιβάλλουν τον κώδικα μέσα σε { }
// Η main είναι η συνάρτηση που εκτελείται όταν τρέχετε το πρόγραμμά σας.
...
func main() { // η κύρια συνάρτηση που εκτελεί την εφαρμογή
	...
	// Η Println είναι και αυτή μια συνάρτηση μέσα απο το πακέτο fmt που εξάγει - τυπώνει μια συμβολοσειρά (strings), η οποία
	// περιβάλλεται από διπλά εισαγωγικά π.χ. "Γειά σου κόσμε", σε μια νέα γραμμή στην οθόνη

	fmt.Println("Γειά σου κόσμε") // εκτελούμε την Println που βρίσκεται
	// μέσα στην fmt βιβλιοθήκη που εισάγαμε

	/* Μπορείτε να μάθετε για την λειτουργία της πληκτρολογώντας << godoc fmt Println >>
	σε ένα τερματικό. Το πρόγραμμά σας, όπως αυτό, μπορείτε να το τρέξετε σε τερματικό γράφοντας π.χ. go run data_type_math.go

	Οι μεταβλητές (Variables) γράφονται στατικά (statically typed), πράγμα που σημαίνει ότι ο τύπος (type) τους δεν μπορεί να αλλάξει (static). Τα ονόματα μεταβλητών πρέπει να ξεκινούν με ένα γράμμα και μπορεί να περιέχουν γράμματα, αριθμούς ή την κάτω παύλα _
	Για τους αριθμούς έχουμε τους παρακάτω τύπους:

	Μια int (intiger - ακαίρεος) είναι ένας θετικός ή αρνητικός αριθμός χωρίς δεκαδικά ψηφία
	...
	*/
	...
	// Στην πραγματικότητα δεν χρειάζεται να καθορίσετε τον τύπο δεδομένων, ούτε χρειάζεστε ερωτηματικό στο τέλος:
	...
	/*
		     ...
		     π.χ. να αλλάξετε το υφηστάμενο randNum που είναι int να το κάνετε
		     randNum = "Hello"

			 Στην Println εκτός απο strings μπορείτε να χρησιμοποιήσετε
			 μεταβλητές (το κενό ιάστημα προστίθεται αυτόματα οπότε δεν χρειάζεται)
	*/
	...
	// το % ή αλλιώς modulus, modulo, remainder είναι το υπόλοιπο μιας διαίρεσης
	...
	fmt.Println("6 / 4 =", 6/4)
	fmt.Println("6 % 4 =", 6%4)
	// το % operator είναι διαθέσιμο μόνο για intigers οπότε δείτε παρακάτω 
