...
// Για να φτιάξετε ένα εκτελέσιμο απο το πρόγραμμά σας και να το μοιραστείτε θα πρέπει να το κάνετε
// compile (μεταγλώττιση):
//  π.χ. go build data_type_math.go
...
func add(x int, y int) int { // δημιουργούμε μια δικιά μας function (συνάρτηση) με ένα όνομα (π.χ. add) η οποία
	// κάνει μια απλή αριθμητική πράξη προσθήκης δύο αριθμών. Η function add παίρνει 2 ορίσματα x και y
	...
	return x + y // προσθέτει δυο αριθμούς τύπου int που θα δοθούν στο add(x,y)

	/* εφόσον τα ορίσματα είναι καί τα δύο int θα μπορούσαμε επίσης να τα γράψουμε με ένα int
	όπως π.χ.
...
func main() { // η κύρια συνάρτηση που εκτελεί την εφαρμογή
	...
	*/
	...
	/*
		...
		δημιουργήσει και το χρησιμοποιούμε για να προσθέσουμε τους δύο int αριθμούς x,y
	...
	*/
	...
	randNum := 1 // Η Go καταλαβαίνει οτι πρόκειται για int
	fmt.Println(randNum)
