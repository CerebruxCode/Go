...
func myCustomMap() {
	...
	*/
	...
	// Ας τα εμφανίσουμε σε 3 σειρές
	for key, value := range superhero {
		fmt.Println("Key:", key, " - Values:", value)
	}
	/*
	   Είναι σημαντικό να γίνει κατανοητό οτι, όπως και με τα slices, τα maps
	   είναι τύποι αναφοράς. Αυτό σημαίνει ότι τα maps παραλαμβάνονται (κλήσεις)
	   απο άλλα func με παραπομπή και όχι απευθείας με τις τιμές τους.

	   Έτσι, κατά τη κλήση των maps σε functions, η Go δεν δημιουργεί ξεχωριστό αντίγραφο
	   των τιμών των maps τοποθετώντας τα σε μια αντίγραφη στοίβα για χρήση απο την εν λόγω
	   function. Έτσι, οι αλλαγές που κάνετε σε ένα map είναι ορατές και στις κλήσεις που γίνονται
	   σε αυτό αλλά και σε οποιεσδήποτε άλλες functions που αναφέρονται σε αυτό.

	   Για αυτό τον λόγο τα maps δεν είναι ασφαλείς για concurency αφού η ταυτόχρονη
	   ανάγνωση και εγγραφή σε maps μπορεί να οδηγήσει σε προβλήματα.
	   Είναι απλά δείκτες και όχι τα πραγματικά δεδομένα που μεταφέρονται.

	   Τέλος θεωρείται μια καλή πρακτική να καθορίζουμε το μέγεθος
	   ενός map όποτε είναι δυνατόν. Δεν είναι μεν υποχρεωτικό, αλλά μπορεί
	   να βελτιώσει την απόδοση, ειδικά αν πρόκειται να εργαστείτε με μεγάλα maps.
	   Επειδή μοιάζουνε με τα slices όσον αφορά τον τρόπο που μεγαλώνουν, προσπαθήστε
	   να δώσετε μια ιδέα στην Go για το πόσο μεγάλο περιμένετε να γίνει το map σας.
	   Μπορεί να συμβάλει στη βελτίωση της απόδοσης του.
	*/
}
