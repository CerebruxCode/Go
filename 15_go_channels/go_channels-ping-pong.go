...
// CHANNELS
/*
 Τα κανάλια παρέχουν έναν τρόπο για δύο go-routines να επικοινωνούν μεταξύ τους
...
// Δημιουργούμε δυο funcs "ping" και "pong" κανάλια που θα προβάλλονται ασταμάτητα

func pinger(c chan string) {
	for i := 0; ; i++ {
		c <- "ping" // σημαίνει αποστολή "ping" στο κανάλι c.
	}
}

func ponger(c chan string) {
	for i := 0; ; i++ {
		c <- "pong" // σημαίνει αποστολή "pong" στο κανάλι c
	}
}
...
func printer(c chan string) {
	for { // Βλέπουμε οτι η for εδω θα εκτελείται για πάντα μέχρι να πατήσουμε
...
pinger και ο ponger προσπαθούν να στείλουν ένα μήνυμα στο κανάλι, θα περιμένουν μέχρι ο printer να
...
func main() {
	...
	var input string
	fmt.Scanln(&input) // πατάμε enter στο πληκτρολόγιο για να σταματήσει το πρόγραμμα
