...
func main() {
	...
	// FILE I/O
	/*
		Η πρότυπη βιβλιοθήκη "io" της Go περιλαμβάνει πολλές υλοποιήσεις των διεπαφών διαχείρησης
		αρχείων για εισαγωγή και εγραφή δεδομένων, συνδέσεων δικτύου, συμπιεστών, κρυπτογράφησης και άλλων.
	*/

	// Δημιουργησε ένα αρχείο με όνομα samp.txt στην διαδρομή απο όπου εκτελείται η εφαρμογή
	// π.χ. αν τρέξουμε την εφαρμογή <<go run enas_fakelos/kodikas.go>> τοτε το αρχείο θα δημιουργηθεί δίπλα απο τον φάκελο
	// "enas_fakelos" και όχι μέσα στον φάκελο δίπλα απο το "kodikas.go"
	...
	file.WriteString("Αυτό είναι ένα τυχαίο κέιμενο")

	// Κλείστε το αρχείο
	file.Close()

	// Προσπήθησε να ανοίξεις το αρχείο. Αν δεν το καταφέρεις πρόβαλε το σφάλμα
