...
Τι θα μάθουμε:
- Στοιχεία της Go
- Πως τρέχουμε τον κώδικά μας
- Πως δημιουργούμε ένα εκτελέσιμο απο τον κώδικά μας
- Πως μπορούμε να κάνουμε compile τον κώδικά μας για όλα τα λειτουργικά συστήματα

Πακέτα

Οι εφαρμογές Go είναι οργανωμένες σε πακέτα. Ένα πακέτο είναι μια 
συλλογή αρχείων κώδικα που βρίσκονται μέσα στον ίδιο φάκελο.

Όλα τα αρχεία κώδικα σε έναν φάκελο πρέπει να έχουν το ίδιο όνομα πακέτου με εξαίερεση
το αρχείο main.go

Όπως θα δούμε σε επόμενα μαθήματα, όταν εισάγεται (import) ένα πακέτο, μόνο οι 
οντότητες του (συναρτήσεις, τύποι, μεταβλητές, σταθερές) των οποίων το όνομα 
αρχίζει με κεφαλαίο γράμμα μπορούν να χρησιμοποιηθούν και να έχουμε πρόσβαση
στις μεθόδους τους.

Το συνιστώμενο στυλ ονομασίας οντοτήτων (ονόματα μεταβλητών, συναρτησεων κλπ) 
στη Go είναι να ονομάζονται χρησιμοποιώντας το "camelCase" στύλ όπου τα ονόματα 
ξεκινάνε με μικρό γράμμα, με εξαίρεση όπως είπαμε πιο πάνω, αυτά που προορίζονται 
να είναι προσβάσιμα σε όλα τα πακέτα και θα πρέπει να ξεκινάνε με κεφαλαίο το 
πρώτο γράμμα. Δηλαδή "CamelCase".
...
(1) Το πρώτο πράγμα που χρειάζεται κάθε πρόγραμμα Go είναι
ένα "package declaration". Το βασικό μας πρόγραμμα θα είναι πάντα
το "main", με το οποίο λέμε στην ουσία στον compiler (μεταγλωττιστή)
να το μεταγλωττίσει ως αυτόνομο εκτελέσιμο πρόγραμμα. Όλα τα
υπόλοιπα Go προγράμματα που θα γράψουμε και δε θα τους δώσουμε
το "main" declaration, χρησιμοποιούνται ως κοινόχρηστες βιβλιοθήκες,
τις οποίες μπορούμε να καλέσουμε στο πρόγραμμά μας με το "import".
...
είτε έρχονται από τις έτοιμες βιβλιοθήκες της Go (Go Standard Libraries).
...
προγράμματος μας. Φυσικά το πρόγραμμά μας μπορεί να μην αποτελείται μόνο απο αυτό το
αρχείο αλλά και πάρα πολλά άλλα αρχεία, βιβλιοθήκες κλπ. Στην περίπτωση όμως
του προγράμματος "package_main", αυτό αποτελείται μόνο απο το "package_main.go" αρχείο.
...
func main() { //(3) Function (main) η κύρια συνάρτηση που εκτελεί την εφαρμογή

	fmt.Println("Γεια σου κόσμε") // (4) εκτελούμε την Println function που βρίσκεται
...
μοιράσουμε με άλλους, θα πρέπει να το κάνουμε compile (μεταγλώττιση) ως εξής:
...
Περισσότερα σχετικά με τα λειτουργικά συστήματα και τις αρχιτεκτονικές
για τις οποίες μπορούμε να δημιουργήσουμε εκτελέσιμα
