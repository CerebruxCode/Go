...
/*
	...
	βιβλιοθήκες (έτοιμες ρουτίνες). Εδώ τις ομαδοποιούμε σε μία import
	χρησιμοποιόντας παρένθεση
*/
import (
	"fmt"
	"math"
...
/*
	...
	Η δήλωση (statement) δεν χρειάζεται να περιβάλλεται από παρενθέσεις ()
	αλλά απαιτούνται οι αγκύλες {}.
*/

func main() {

	fmt.Println("Καλώς όρησες στην Go!")
	...
	/* η Intn επιστρέφει intiger ψευδο-τυχαίο αριθμό απο 0 έως n*/

	fmt.Println(math.Pi)
	/*exported/un-exported: Όταν ξεκινάει με κεφαλαία τοτε η υπορουτίνα είναι
	προσβάσιμη απο άλλα αρχεία του go*/
	...
	// είναι αληθείς οι υπόλοιπες συνθήκες θα αγνοηθούν:
	...
	/*
		...
		Ο διακόπτης Go είναι σαν τον C, C ++, Java, JavaScript και PHP,
		εκτός από το ότι το Go εκτελεί μόνο την επιλεγμένη περίπτωση,
		όχι όλες τις περιπτώσεις που ακολουθούν. Στην πραγματικότητα,
		η δήλωση σπάσει που απαιτείται στο τέλος κάθε περίπτωσης σε αυτές τις γλώσσες
		παρέχεται αυτόματα στο Go.

		Μια άλλη σημαντική διαφορά είναι ότι οι περιπτώσεις switch του Go
...
func myOS() {
	...
	// για να δεις όλα τα διαθέσ. λειτουργικά της GOOS τρέξε "go tool dist list"
	case "darwin":
		fmt.Println("OS X.")
	case "linux":
		fmt.Println("Linux.")
